...
class DFSP(nn.Module):
    ...
    def visual(self, x: torch.Tensor):
        ...
        #print(x.size())
        #print(self.clip.visual.proj.size)
    ...
    def forward(self, batch_img, idx):
        ...
        if self.config.fusion in ["BiFusion", "img2txt"]:
            print(normalized_img.unsqueeze(1).size())
            print(idx_text_features.permute(0,2,1).size())
            print((normalized_img.unsqueeze(1) @ idx_text_features.permute(0,2,1)).size())
            logits = (
