...
def loss_calu(predict, target, config):
    ...
    loss_logit_df = loss_fn(torch.log(prob+1e-9), batch_target)
    loss_att = loss_fn(torch.log(prob_att+1e-9), batch_attr)
    loss_obj = loss_fn(torch.log(prob_obj+1e-9), batch_obj)
